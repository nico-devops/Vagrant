...
IP_HAPROXY=$(dig +short srv-haproxy)
IP_MASTER=$(dig +short srv-master)

# Function
# prepare and configure kubespray
prepare_kubespray(){
...
for srv in $(cat /etc/hosts | grep srvk | awk '{print $2}');do
...
ssh -o StrictHostKeyChecking=no -i /home/vagrant/.ssh/id_rsa vagrant@${IP_MASTER} "sudo cat /etc/kubernetes/admin.conf" >/home/vagrant/.kube/config
}
